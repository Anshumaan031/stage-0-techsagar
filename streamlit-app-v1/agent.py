...
def get_system_prompt(num_companies):
    return f'''You are an expert at researching Indian technology startups.
    Your task is to:
    ...
    3. Limit extraction to {num_companies} most promising startups per technology area
...
def create_search_agent(num_companies):
    agent = Agent(
        'openai:gpt-4',
        deps_type=CompanySearchDependencies,
        result_type=CompanySearchResult,
        system_prompt=get_system_prompt(num_companies)
    )

    # Define and attach the tool to the agent
    @agent.tool
